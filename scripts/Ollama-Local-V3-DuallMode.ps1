<#
  Ollama Local V3 — aplicativo portátil para Windows 11 (modo Runspace ou APM).
  ===========================================================
  Política de rede: esta interface usa exclusivamente http://127.0.0.1:11434.

  MODO DE ENVIO (parâmetro -Mode):
    Runspace  — Invoke-RestMethod em um runspace separado + timer 300 ms
                (equivalente ao commit a9ea81f da Cláudia; cancelamento via Stop)
    APM       — HttpWebRequest.BeginGetResponse + timer 200 ms
                (equivalente ao commit facf1ba; cancelamento via Abort)

  Uso:
    .\Ollama-Local-V3-DuallMode.ps1                    # modo padrão: Runspace
    .\Ollama-Local-V3-DuallMode.ps1 -Mode APM          # modo BeginGetResponse
    .\Ollama-Local-V3-DuallMode.ps1 -Mode Runspace

  O arquivo deve ser gravado em UTF-8 com BOM (PowerShell 5.1 do Windows 11).
#>

param(
    [ValidateSet('Runspace', 'APM')]
    [string]$Mode = 'Runspace'
)
...
# ----------------------------------------------------------------------------
# Modo APM — HttpWebRequest.BeginGetResponse (commit facf1ba)
# ----------------------------------------------------------------------------
function Send-Prompt-Apm {
    $model = [string]$modelSelector.SelectedItem
    $prompt = $promptBox.Text.Trim()
    if (-not $model) { Set-AppStatus 'Escolha um modelo local antes de enviar.' $true; return }
    if (-not $prompt) { Set-AppStatus 'Escreva uma mensagem antes de enviar.' $true; return }
    if ($script:ActiveRequest -or $script:ActiveRunspace) {
    ...
    try {
        ...
        Set-AppStatus "Modo APM — gerando com $model em segundo plano."
...
function Complete-LocalGeneration-Apm {
...
function Cancel-LocalGeneration-Apm {
    if (-not $script:ActiveRequest) { return }
    try { $script:ActiveRequest.Abort() } catch { }
    $generationTimer.Stop()
    Reset-ActiveState
    Add-Transcript -Text 'Geração cancelada (Abort). O modelo pode permanecer carregado até usar "Liberar VRAM".' -Color [System.Drawing.Color]::DarkGoldenrod
    Set-AppStatus 'Geração cancelada.'
    Update-LoadedStatus
}

# ----------------------------------------------------------------------------
# Modo Runspace — Invoke-RestMethod em runspace (commit a9ea81f)
# ----------------------------------------------------------------------------
function Send-Prompt-Runspace {
    $model = [string]$modelSelector.SelectedItem
    $prompt = $promptBox.Text.Trim()
    if (-not $model) { Set-AppStatus 'Escolha um modelo local antes de enviar.' $true; return }
    if (-not $prompt) { Set-AppStatus 'Escreva uma mensagem antes de enviar.' $true; return }
    if ($script:ActiveRunspace -or $script:ActiveRequest) {
        Set-AppStatus 'Já existe uma resposta em andamento. Aguarde ou cancele a geração atual.' $true
        return
    }
    ...
    try {
        ...
        $scriptCode = {
            ...
            try {
                $r = Invoke-RestMethod -Uri $RequestUri -Method Post `
                    -ContentType 'application/json' -Body $RequestBody -TimeoutSec 600 -ErrorAction Stop
        ...
        $ps.Runspace = $rs
        [void]$ps.AddScript($scriptCode).AddArgument($uri).AddArgument($payload)
        $handle = $ps.BeginInvoke()

        $script:ActiveRunspace = @{ Runspace = $rs; PowerShell = $ps; Handle = $handle }
        $script:ActiveModel = $model
        $sendButton.Enabled = $false
        $cancelButton.Enabled = $true
        $promptBox.Enabled = $false
        Set-AppStatus "Modo Runspace — gerando com $model em segundo plano."
        Add-Transcript -Text "Você — $prompt" -Color $script:Ink
        $promptBox.Clear()
        $generationTimer.Start()
    }
...
function Complete-LocalGeneration-Runspace {
    $state = $script:ActiveRunspace
    if (-not $state -or -not $state.Handle.IsCompleted) {
        return
    }
    ...
    $ps = $state.PowerShell
    try {
        $response = [string]($ps.EndInvoke($state.Handle) | Select-Object -Last 1)
        if (-not $response) {
            throw 'O runspace concluiu sem resposta (modelo interrompido ou erro silencioso).'
        }
        Add-Transcript -Text "$($script:ActiveModel) — $($response.Trim())" -Color $script:Accent
        Set-AppStatus 'Resposta local concluída.'
    }
    catch {
        Add-Transcript -Text "Erro — $($_.Exception.Message)" -Color [System.Drawing.Color]::Firebrick
...
function Cancel-LocalGeneration-Runspace {
    $state = $script:ActiveRunspace
    if (-not $state) { return }
    ...
    $generationTimer.Stop()
    Reset-ActiveState
    Add-Transcript -Text 'Geração cancelada (Stop do runspace). O modelo pode permanecer carregado até usar "Liberar VRAM".' -Color [System.Drawing.Color]::DarkGoldenrod
    Set-AppStatus 'Geração cancelada.'
    Update-LoadedStatus
}
...
function Send-Prompt {
    if ($Mode -eq 'APM') { Send-Prompt-Apm } else { Send-Prompt-Runspace }
}

function Complete-LocalGeneration {
    if ($Mode -eq 'APM') { Complete-LocalGeneration-Apm } else { Complete-LocalGeneration-Runspace }
}

function Cancel-LocalGeneration {
    if ($Mode -eq 'APM') { Cancel-LocalGeneration-Apm } else { Cancel-LocalGeneration-Runspace }
}
...
$form.Text = "Ollama Local — Windows 11 — modo $Mode"
...
$statusLabel.Text = "Modo $Mode — conectando somente a 127.0.0.1:11434..."
...
$generationTimer.Interval = if ($Mode -eq 'APM') { 200 } else { 300 }
...
$form.Add_FormClosing({
    ...
    if ($script:ActiveRunspace) { try { $script:ActiveRunspace.PowerShell.Stop() } catch { } }
})

Add-Transcript -Text "Interface local iniciada em modo $Mode. Ela só se comunica com o Ollama em 127.0.0.1:11434." -Color $script:Accent
Refresh-Models
